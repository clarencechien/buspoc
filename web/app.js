...
function addPoint(p,label,kind){const el=document.createElement('div');el.className='point-marker '+kind;el.textContent=label;el.title=p.name||'我的位置';points.push(new mapboxgl.Marker({element:el}).setLngLat([p.lon,p.lat]).addTo(map));}
...
function initMap(){
  ...
  try {
    ...
    map.on('load',async()=>{$('#map-empty').hidden=true;message('');drawPoints();render();corridor?.draw();const currentMap=map;try{const layer=await createBusLayer(currentMap,()=>message('3D 車體載入失敗，保留公車標籤。'));if(map===currentMap){busLayer=layer;render();}}catch{if(map===currentMap)message('3D 車體載入失敗，保留公車標籤。');}});
...
$('#reference').onchange=()=>{if($('#reference').value==='me'&&!me){$('#reference').value='origin';$('#locate').click();}popup?.remove();render();};$('#direction').onchange=()=>{popup?.remove();render();};$('#refresh').onclick=refresh;$('#mode').onchange=()=>{vehicles=[];fetched=null;hasSnapshot=false;popup?.remove();$('#refresh').disabled=false;refresh();};
