...
function initMap(){
  ...
  try {
    ...
    map.on('load',()=>{$('#map-empty').hidden=true;message('');drawPoints();render();});
...
setInterval(()=>{if(!document.hidden&&$('#mode').value==='live')refresh();},30000);
setInterval(()=>{if(!document.hidden)render();},10000);
initRoutes();updateJourney();initMap();render();refresh();if(storageWarning)message(storageWarning);
