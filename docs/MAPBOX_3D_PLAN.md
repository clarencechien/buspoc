...
依最新要求，TDX token 保存於 localStorage，與路線支援 JSON 匯入／匯出；Actions 只支援注入 Mapbox 公開 token，不公開 TDX Secrets。
...
- 可編輯路線、A/B、Mapbox 公開 token；設定驗證及本機儲存。
- TDX RealTimeByFrequency 由瀏覽器直接請求；使用者貼 Access Token，存於 localStorage；支援逾時、401、403、429、CORS 錯誤提示。
